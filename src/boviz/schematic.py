...
def plot_initial_particle_schematic(
    ...
    particle_labels: list = None,
    particle_colors: Union[str, list] = "white",
):
    """
    绘制初始粒子分布的示意图。
    ...
    for i in range(len(coordinates)):
        current_color = particle_colors[i] if isinstance(particle_colors, list) else particle_colors
        circle = plt.Circle(
        ...
        if show_particle_labels:
            label_text = particle_labels[i] if particle_labels else rf"$\text{{P}}_{{{i + 1}}}$"
            plt.text(
                coordinates[i][0],
                coordinates[i][1],
                label_text,
...
def plot_initial_superellipse_schematic(
    ...
    if not font_style:
        if font_weight == 'bold':
            ...
            raise ValueError("Invalid font_weight.")
    elif font_style == 'sans':
        if font_weight == 'bold':
            set_sans_style(bold=True)
        elif font_weight == 'normal':
